...
const createContext = (trial, driver, useLocal) => {
    const tableInfo = Object.assign({}, tables.get(trial.table), { data: undefined });
    return JSON.stringify(Object.assign({}, trial, tableInfo, { driver, tables: [...tables.keys()], useLocal }));
};
...
const trials = getTrials(searchTerms).sort(sortTrials);
if (!trials.length) {
...
const drivers = require('./drivers')(useLocal);
...
for (const trial of trials) {
    ...
    for (const driver of drivers.keys()) {
        ...
        const ctx = createContext(trial, driver, useLocal);
